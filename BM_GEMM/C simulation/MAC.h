...
void BMMul(ap_uint<W> a, ap_uint<W> b, ap_int<Kadd-WI-WT> &mult, ap_uint<2> mode){
...
template<int Ebit>
void GetEbeta2(ap_uint<Ebit> &ebias, ap_uint<1> &flag, SEXP_T betaAB, SEXP_T &betaC, unsigned k){
	ap_int<Ebit+1> bias = ap_int<Ebit+1>(betaAB - betaC);
	flag = bias[Ebit];
	ebias = bias>=0? bias(Ebit-1,0) : (~ap_uint<Ebit>(bias(Ebit-1,0))+1);
	betaC = bias>=0 || k==0? betaAB : betaC;
//	printf("--- GetEbeta -- \n");
//	printf("           betaAB: %d\n",betaAB.to_int());
//	printf("           flag:%d\t ebias:%d\t betaC:%d\n",flag.to_int(), ebias.to_int(), betaC.to_int());
}
...
template<int Ebit>
void BMInterAccum(ap_int<Kadd> psum, ap_int<Kadd> &Acc, ap_uint<Ebit> ebias, ap_uint<1> flag){
	ap_int<Kadd> A = psum;
	ap_int<Kadd> B = Acc;
	ap_uint<Ebit> Ka = flag? ebias : ap_uint<Ebit>(0);
